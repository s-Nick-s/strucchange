...
%\VignetteIndexEntry{strucchangeC: An R Package for Testing for Structural Change in Linear Regression Models}
%\VignetteDepends{strucchangeC}
%\VignetteKeywords{structural change, CUSUM, MOSUM, recursive estimates, moving estimates, monitoring, R, S}
%\VignettePackage{strucchangeC}

\title{\texttt{strucchangeC}: An \textsf{R}  Package for Testing for Structural
...
\begin{abstract}
This introduction to the \textsf{R} package \texttt{strucchangeC} is a (slightly)
modified version of
...
has been realized in {\tt strucchangeC}.
...
implemented in {\tt strucchangeC} and give examples for each. Section
...
The data is
available in the {\tt strucchangeC} package: it can be loaded and a
...
In this paper, we have described the {\tt strucchangeC} package that implements
...
In addition to these methods for the linear regression model, the \texttt{strucchangeC}
...
The corresponding functions in \texttt{strucchangeC} are presented in
...
implementation in the {\tt strucchangeC} package.\\
